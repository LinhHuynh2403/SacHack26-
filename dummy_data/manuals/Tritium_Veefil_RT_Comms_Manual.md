...
**Target Part:** Main Processor Board
...
When a failure occurs in the Main Processor Board, the system will typically issue the error code `TR-SYS-OFF` to the central operations dashboard. 
...
*   Replacement Main Processor Board
...
2. If `TR-SYS-OFF` persists after boot sequence, proceed with physical inspection.
...
3. Locate the Main Processor Board inside the chassis.

**Step 4.3: Diagnostic Testing**
1. Disconnect the logic/control cables from the Main Processor Board.
...
3. *Recommendation: Hard reboot sequence, serial interface diagnostic*

**Step 4.4: Replacement**
1. Unplug the main harness. 
2. Slide the Main Processor Board out of the bracket. 
...
3. Ping the charger from the diagnostic technician app to verify `TR-SYS-OFF` has cleared.
