...
**Target Part:** NFC/RFID Reader Module (Nayax VPOS)
...
When a failure occurs in the NFC/RFID Reader Module (Nayax VPOS), the system will typically issue the error code `ERR-PAY-01` to the central operations dashboard. 
...
*   Replacement NFC/RFID Reader Module (Nayax VPOS)
...
2. If `ERR-PAY-01` persists after boot sequence, proceed with physical inspection.
...
3. Locate the NFC/RFID Reader Module (Nayax VPOS) inside the chassis.

**Step 4.3: Diagnostic Testing**
1. Disconnect the logic/control cables from the NFC/RFID Reader Module (Nayax VPOS).
...
3. *Recommendation: Reset reader and test cellular pulse*

**Step 4.4: Replacement**
1. Unplug the main harness. 
2. Slide the NFC/RFID Reader Module (Nayax VPOS) out of the bracket. 
...
3. Ping the charger from the diagnostic technician app to verify `ERR-PAY-01` has cleared.
