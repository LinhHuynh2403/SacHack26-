...
**Target Part:** 30A Contactor Board
...
When a failure occurs in the 30A Contactor Board, the system will typically issue the error code `CP-ERR-CON` to the central operations dashboard. 
...
*   Replacement 30A Contactor Board
...
2. If `CP-ERR-CON` persists after boot sequence, proceed with physical inspection.
...
3. Locate the 30A Contactor Board inside the chassis.

**Step 4.3: Diagnostic Testing**
1. Disconnect the logic/control cables from the 30A Contactor Board.
...
3. *Recommendation: Measure voltage drop across poles, replace contactor*

**Step 4.4: Replacement**
1. Unplug the main harness. 
2. Slide the 30A Contactor Board out of the bracket. 
...
3. Ping the charger from the diagnostic technician app to verify `CP-ERR-CON` has cleared.
