...
**Component Category:** Thermal
**Target Part:** Stall Temperature Sensor
...
When a failure occurs in the Stall Temperature Sensor, the system will typically issue the error code `VC-STC-TMP` to the central operations dashboard. 
...
*   Replacement Stall Temperature Sensor
...
2. If `VC-STC-TMP` persists after boot sequence, proceed with physical inspection.
...
3. Locate the Stall Temperature Sensor inside the chassis.

**Step 4.3: Diagnostic Testing**
1. Disconnect the logic/control cables from the Stall Temperature Sensor.
...
3. *Recommendation: Replace thermistor on the cable handle logic board*

**Step 4.4: Replacement**
1. Unplug the main harness. 
2. Slide the Stall Temperature Sensor out of the bracket. 
...
3. Ping the charger from the diagnostic technician app to verify `VC-STC-TMP` has cleared.
