...
When a failure occurs in the CHAdeMO Connector, the system will typically issue the error code `TR-CHA-CHd` to the central operations dashboard. 
...
2. If `TR-CHA-CHd` persists after boot sequence, proceed with physical inspection.
...
3. Locate the CHAdeMO Connector inside the chassis.

**Step 4.3: Diagnostic Testing**
1. Disconnect the logic/control cables from the CHAdeMO Connector.
...
3. *Recommendation: Check proximity pilot pin impedance*

**Step 4.4: Replacement**
1. Unplug the main harness. 
2. Slide the CHAdeMO Connector out of the bracket. 
...
3. Ping the charger from the diagnostic technician app to verify `TR-CHA-CHd` has cleared.
